...
function _M:getCombatDam()
	local dam = 0
	local inv = self:getInven("MAIN_HAND")
	if inv then
		local o = inv[1] or nil
		if o then
			--game.logSeen(self, "getCombatDam: MAIN_HAND: %s", o.name)
			dam = o.combat.dam
		else
			dam = self.combat.dam
		end
...
function _M:postUseTalent(ab, ret)
	...
	if not ab.no_energy == true then self:useEnergy() end
